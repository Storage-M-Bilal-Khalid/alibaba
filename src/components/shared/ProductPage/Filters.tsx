...
import { Checkbox, CheckboxGroupList, Label } from './CheckBoxOrLabelOrCheckBoxList';
import { PriceRangeFilter } from './PriceRangeFilter';
import { CategoryData, conditionFilter, productCategories, ratingFilter } from './configuration/configuration';
  ...
  return (
    <div className={`overflow-hidden`}>
      <h2 id={`accordion-heading-${item.id}`}>
        <div
          className="flex items-center justify-between w-full focus:outline-none focus:ring-0 border-t border-[#dee2e7] pt-3 pb-3 pl-2 pr-2 cursor-pointer"
          onClick={() => onToggle(item.id)}
          aria-expanded={isOpen}
          aria-controls={`accordion-body-${item.id}`}
        >
          <span className="text-sm font-medium text-[#1c1c1c] text-shadow-accent">{item.title}</span>
      ...
      <div
        ...
        className={`overflow-hidden ${isOpen ? 'max-h-screen opacity-100 pb-5 pl-2 pr-2' : 'max-h-0 opacity-0'}`}
      >
            ...
            ratingFilter.map(rating => (
              <div key={rating.id} className="flex items-center space-x-2">
                <Checkbox
                  ...
                  onCheckedChange={(isChecked) => handleCheckboxChange(rating.id, 'ratings', isChecked)}
                />
  ...
  return (
    <div className="text-sm">
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        <Button
          type="submit"
          className="mt-4 w-full p-2 rounded-[5] bg-white text-[#0A74FF] border-1 border-[#DEE2E7] hover:tracking-wide transition-all duration-300 ease-in-out  hover:cursor-pointer"
          variant="default"
        >
          Apply
        </Button>
